...
def capitalize(instring):

    """ Capitalizes first character in string and lowers rest.
...
campuses = db_info.campus_names
tables = db_info.table_names
...
query = "SELECT last_name, first_name FROM " + table + " WHERE campus='{}'".format(campus)
prof_list.execute(query)


# Now we have a list of all the professor names from this campus.
# Which department are they in? None of the online databases have that
# info, so use google instead.
i = 0
overall_rating = []
dept = []
names = []
for (last_name, first_name) in prof_list:
    ...
    name =capitalize(first_name.strip())+' '+capitalize(last_name.strip()) 
    rmp_search = 'https://www.ratemyprofessors.com/search.jsp?query='+name.replace(' ','+')
    #    rmp_search = ('https://www.ratemyprofessors.com/search.jsp?queryoption=HEADER&' +
    #                  'queryBy=teacherName&schoolName=University+of+California'+capitalize(campus) +
    #                  '&query='+first_name+'+'+last_name)
    #^^ Prof may have just switched universities and won't have rmp account associated with new (current).
    ...
    # If the overall rating on rmp is 0.0 then there are no ratings at all. Otherwise look for rating(time).
